...
const BAND: Record<string, string> = {
  below: "var(--color-below)", slightly_below: "var(--color-below)",
  at: "var(--color-at)", above: "var(--color-above)", strong: "var(--color-above)",
};
...
export function PercentileTrack({ dist, you, band, animate = true }:
  { dist: Dist; you?: number | null; band?: string; animate?: boolean }) {
  const min = dist.p25 * 0.8, max = dist.p90 * 1.08, span = Math.max(max - min, 1);
  ...
  return (
    <div>
      ...
      <div className="relative h-14 rounded-md bg-inset ring-1 ring-line overflow-hidden">
        ...
        <div className="absolute inset-y-0 bg-white/[0.055]"
        ...
        {you != null && (
          <div className={`absolute inset-y-0 ${animate ? "marker-slide" : ""}`} style={{ left: `${pos(you)}%` }}>
            <div className="absolute inset-y-0 -translate-x-1/2" style={{ width: 3, background: c, boxShadow: "0 0 0 2px var(--color-ink)" }} />
          </div>
        )}
      </div>
      ...
      {you != null && (
        <div className="relative h-0">
          <div className={`absolute -top-[70px] -translate-x-1/2 ${animate ? "marker-slide" : ""}`} style={{ left: `${youL}%` }}>
            <div className="rounded px-2 py-0.5 text-[11px] font-semibold tnum whitespace-nowrap"
