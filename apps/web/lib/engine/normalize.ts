// Comp normalization — bucket messy free-text into the benchmark cell. Pure
// functions, ported 1:1 from platform/src/comp/normalize-comp.mjs so the web
// (Neon) and platform (SQLite) paths classify identically. Runs at write- and
// query-time. SECURITY: metro never encodes a site/base; clearance is a TIER.
...
export const ROLE_SYNONYMS: { family: string; patterns: string[] }[] = [
  {
    family: 'cyber',
    patterns: [
      'cybersecurity',
  ...
  {
    family: 'network_engineer',
    patterns: ['network engineer', 'network administrator', 'network architect'],
  },
  { family: 'database_admin', patterns: ['database administrator', 'dba', 'database engineer'] },
  {
    family: 'systems_engineer',
    patterns: [
      'systems engineer',
      'system engineer',
      'systems integration',
  ...
  {
    family: 'test_engineer',
    patterns: ['test engineer', 'quality assurance', 'qa engineer', 'sdet', 'test automation'],
  },
  {
    family: 'hardware_engineer',
    patterns: [
      'hardware engineer',
      'fpga',
      'embedded engineer',
  ...
  {
    family: 'aerospace_engineer',
    patterns: ['aerospace engineer', 'mechanical engineer', 'flight software', 'gnc', 'propulsion'],
  },
  {
    family: 'systems_architect',
    patterns: [
      'solutions architect',
  ...
  {
    family: 'help_desk',
    patterns: ['help desk', 'service desk', 'desktop support', 'it support', 'field service'],
  },
];

export function roleFamily(raw: string | null | undefined): string {
  const c = canon(raw);
  if (!c) return 'other';
  const all = ROLE_SYNONYMS.flatMap((r) =>
    r.patterns.map((p) => ({ p: canon(p), family: r.family }))
  ).sort((a, b) => b.p.length - a.p.length);
  for (const { p, family } of all) if (c.includes(p)) return family;
  return 'other';
}

export const METRO_ALIASES: { metro: string; patterns: string[] }[] = [
  {
    metro: 'dc_metro',
    patterns: [
      'washington dc',
  ...
  {
    metro: 'colorado_springs',
    patterns: ['colorado springs', 'peterson', 'schriever', 'cheyenne mountain', 'norad'],
  },
  ...
  {
    metro: 'national_remote',
    patterns: ['remote', 'telework', 'anywhere', 'work from home', 'wfh'],
  },
];

export function metroBucket(raw: string | null | undefined): string {
  const c = canon(raw);
  if (!c) return 'other';
  const padded = ` ${c} `;
...
export function yoeBand(years: number | string | null | undefined): string | null {
...
export function clearanceTier(raw: string | null | undefined): string | null {
  const c = canon(raw);
  if (!c) return null;
  if (
    c.includes('poly') ||
    c.includes('full scope') ||
    c.includes('fsp') ||
    c.includes('ci poly') ||
    c.includes('lifestyle')
  )
    return 'ts_sci_poly';
  if (c.includes('sci') || c.includes('ts sci') || c.includes('tssci') || c.includes('sap'))
    return 'ts_sci';
  if (
    c.includes('top secret') ||
    c === 'ts' ||
    c.includes(' ts ') ||
    c.startsWith('ts ') ||
    c.endsWith(' ts')
...
export function customerSector(raw: string | null | undefined): string | null {
  const c = canon(raw);
  if (!c) return null;
  if (
    ['nsa', 'cia', 'nga', 'nro', 'dia', 'odni', 'fbi', 'intelligence community', ' ic '].some((k) =>
      ` ${c} `.includes(k)
    )
  )
    return 'ic';
...
export function lcatLevel(raw: string | null | undefined): string | null {
  const c = canon(raw);
  if (!c) return null;
...
export function primeSub(raw: string | null | undefined): string | null {
  const c = canon(raw);
  if (!c) return null;
